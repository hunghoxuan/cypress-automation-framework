...
export default class AssertStoredAttributeNotEquals extends Action  {
    command = Commands.AssertStoredAttributeNotEquals;
     ...
     run(params) {
        ...
        this.getElementAttribute(this.identifier, this.valueArray[0]).then((value) => {
            expect(value).not.to.equal(this.getAttribute(this.valueArray[1]));
        });
        return this.app;
    }
}
